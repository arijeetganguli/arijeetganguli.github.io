export const sanari = {
  ...
  portfolioHover() {
    const sanari_tm_portfolio_animation_wrap = document.querySelectorAll(
        ".sanari_tm_portfolio_animation_wrap"
      ),
      sanari_tm_portfolio_titles = document.querySelector(
        ".sanari_tm_portfolio_titles"
      );
    sanari_tm_portfolio_animation_wrap.forEach((element) => {
      element.addEventListener("mousemove", () => {
        let title = element.getAttribute("data-title"),
          category = element.getAttribute("data-category");
        if (title) {
          sanari_tm_portfolio_titles.classList.add("visible");
          sanari_tm_portfolio_titles.innerHTML =
            title + '<span class="work__cat">' + category + "</span>";
        }
        document.addEventListener("mousemove", (e) => {
          sanari_tm_portfolio_titles.style.left = `${e.clientX - 10}px`;
          sanari_tm_portfolio_titles.style.top = `${e.clientY + 25}px`;
        });
      });
      element.addEventListener("mouseleave", () => {
        sanari_tm_portfolio_titles.classList.remove("visible");
      });
    });
  },
};
